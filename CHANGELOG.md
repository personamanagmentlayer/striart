...
### Documentation
- **`README.md` (racine) devient la version anglaise par défaut** — c'est le fichier que GitHub et npm affichent sans action de l'utilisateur ; l'ancien `README.md` (français) est renommé `README.fr.md` et reste la référence de contenu (mention explicite dans les deux fichiers). `scripts/sync-public.ts` et `docs/fr/README.md`/`docs/en/README.md` recalés sur les nouveaux chemins.
- **Documentation restructurée : `docs/fr/` + `docs/en/`, README réduits en vitrine.** Les README (fr+en, ~650 lignes chacun) portaient toute la doc ; ils sont ramenés à l'essentiel (piliers, tableau comparatif, installation, démarrage express, sommaire, règles d'or) et le détail vit désormais dans 10 pages par langue (démarrage, architecture, commandes, configuration, branches et pipeline, modes d'exécution, plans, MCP, projets volumineux, dépannage). Trois lacunes comblées au passage : le prérequis « repo Git avec au moins un premier commit » (jamais écrit — le cas du projet vierge n'était couvert nulle part), la contrainte « le repo principal doit être positionné sur `targetBranch` » (`TARGET_BRANCH_MISMATCH`/`MAIN_DIRTY`, appliquée par le code mais découverte à l'exécution seulement), et des exemples explicites de branches non-`main` (`dev`, `master`, pipeline `dev` → `main`). `sync-public.ts` : sections `distribution` des pages de démarrage ajoutées aux transformations d'export ; le renvoi public de CONTRIBUTING pointe sur `docs/fr/`.
